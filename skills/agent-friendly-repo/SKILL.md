...
Turn a GitHub repo into one where the agent completion path — commit → push → PR → `gh pr merge --auto --squash` — actually lands, with **CI green for everyone** and **no human review gate** (agents can't approve their own PRs).
