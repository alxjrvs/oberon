...
Reference for the `agent-friendly-repo` skill. SKILL.md says when to read which slice.
...
- Only worth adding where the repo actually has gitignored files a build needs.
...
it, using the **official** `github/gh-stack` extension (public preview since 2026-07-30). Install with
`gh extension install github/gh-stack`. Beware `gh ext search stack`: four community
